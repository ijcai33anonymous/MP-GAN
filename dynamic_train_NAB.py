...
import random
import json
from base_dataset import FeatDataLayer, LoadDataset
from base_models import _netD, _netG,_param, LINEAR_LOGSOFTMAX, weights_init
import classifier_voting

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', default='NABird', type=str) # NABird
...
parser.add_argument('--n_u_sample',  type=int, default=450)
        ...
        for _ in range(1):
            ...
            for part in range(parts):
                ...
                if opt.REG_Wz_LAMBDA != 0:
                    ...
                    reg_Wz_loss = reg_Wz_loss + Wz.pow(2).sum(dim=0).sqrt().sum().mul(opt.REG_Wz_LAMBDA)
...
def train_classifier(opt, param, dataset, netGs, generalized=True):
    ...
    classifier_voting.CLASSIFIER(opt, clfs,
