...
class LoadDataset(object):
    def __init__(self, opt):
        ...
        self.part_cls_centrild = np.zeros([train_cls_num, part_num, 512]).astype(np.float32)
        for i in range(train_cls_num):
        ...
        self.mean_inter_dist = np.zeros([train_cls_num, part_num]).astype(np.float32)
        self.nearest_intra_dist = np.zeros([train_cls_num, part_num]).astype(np.float32)
        ...
        for i in range(train_cls_num):
            for p in range(part_num):
                ...
                self.mean_inter_dist[i][p] = np.mean(inter_dists)
                ...
                for y in range(train_cls_num):
                    ...
                    if dist != 0 and dist < nearest_dist:
                        nearest_dist = dist
                self.nearest_intra_dist[i][p] = nearest_dist

        intra_inter_ratia = self.nearest_intra_dist/self.mean_inter_dist
